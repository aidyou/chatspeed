  ...
  async function extractWithReadability() {
    try {
      ...
      if (!article || !article.content || article.content.length < 100) {
        console.warn('Readability extracted insufficient content, length:', article?.content?.length || 0)
      ...
      console.warn('Readability extraction failed, falling back to custom extractor:', error.message)
      throw error
    }
  }
        ...
        if (useReadabilityFirst && tryCount === 0) {
          try {
            ...
            if (generic_content_rule.format === 'text') {
              ...
              finalContent = turndownService.turndown(readabilityResult.content)
            ...
            console.log('Readability failed, falling back to custom extractor:', readabilityError.message)
