...
impl ContextCompressor {
    ...
    pub async fn compress(&self, messages: &[WorkflowMessage]) -> Result<String, WorkflowEngineError> {
        ...
        let last_summary_idx = messages.iter().rposition(|m| {
            m.metadata.as_ref().map_or(false, |meta| meta["type"] == "summary")
        });
        ...
        let purified_history: Vec<serde_json::Value> = incremental_messages.iter()
            .filter(|m| {
            ...
            .map(|m| {
                serde_json::json!({ "role": m.role, "content": m.message })
            }).collect();
        ...
        let summary = self.extract_fact_from_history(purified_history, user_prompt).await?;
        Ok(summary)
    }

    async fn extract_fact_from_history(&self, history_json: Vec<serde_json::Value>, user_prompt: &str) -> Result<String, WorkflowEngineError> {
        ...
        let chat_interface = {
            ...
            let chat_map = chats_guard.entry(protocol).or_insert_with(std::collections::HashMap::new);
            chat_map.entry("compressor".to_string()).or_insert_with(|| {
                crate::create_chat!(self.chat_state.main_store)
            }).clone()
        };

        log::info!("ContextCompressor: Executing incremental compression...");

        let result = chat_interface.chat(
            self.provider_id,
            &self.model,
            ...
            |_| {} 
        ).await.map_err(WorkflowEngineError::Ai)?;
