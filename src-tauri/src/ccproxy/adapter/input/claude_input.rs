...
pub fn from_claude(req: ClaudeNativeRequest) -> Result<UnifiedRequest, anyhow::Error> {
    let messages = req
        .messages
        .into_iter()
        .map(|msg| {
            let role = match msg.role.as_str() {
                ...
                _ => anyhow::bail!("Invalid or missing role in Claude message"),
            };
            let content = msg
                .content
                .into_iter()
            ...
            Ok(UnifiedMessage { role, content, reasoning_content: None })
        })
        .collect::<Result<Vec<_>, _>>()?;
