...
pub fn start_text_monitor(app: AppHandle, force: Option<bool>) -> Result<(), String> {
    ...
    let is_start = if let Some(force) = force {
        ...
        if let Ok(c) = config_store
            .clone()
            .lock()
            .map_err(|e| t!("toolbar.failed_to_lock_config_store", error = e.to_string()))
        {
    ...
    if check_text_selection_permission().is_ok() {
        ...
        // 在异步块之前获取 receiver
        let mut rx = {
            // map_err for Mutex lock error
            let monitor_guard = monitor_clone.lock().map_err(|e| {
                t!("toolbar.failed_to_lock_monitor", error = e.to_string()).to_string()
            })?;
        ...
        // 启动监控
        if let Ok(monitor_guard) = monitor
            .lock()
            .map_err(|e| t!("toolbar.failed_to_lock_monitor", error = e.to_string()))
        {
            monitor_guard
                .start()
                .map_err(|e| t!("toolbar.failed_to_start_monitor", error = e).to_string())?;
        } else {
...
pub fn stop_text_monitor(monitor: State<Arc<Mutex<TextMonitorManager>>>) -> Result<(), String> {
        ...
        .map_err(|e| t!("toolbar.failed_to_lock_monitor", error = e.to_string()))
    {
        monitor_guard
            .stop()
            .map_err(|e| t!("toolbar.failed_to_stop_monitor", error = e).to_string())?;
    }
    Ok(())
}
