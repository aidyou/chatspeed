        ...
        for file in files:
            if file.endswith(('.vue', '.js', '.ts')):
                ...
                with open(filepath, 'r', encoding='utf-8') as f:
                    ...
                    matches1 = re.findall(r'\bt\s*\(\s*["`\']([a-zA-Z][a-zA-Z0-9_.]*)["`\']\s*(?:,|\))', content, re.DOTALL)
                    # For $t('key') or $t("key") or $t(`key`)
                    matches2 = re.findall(r'\$t\s*\(\s*["`\']([a-zA-Z][a-zA-Z0-9_.]*)["`\']\s*(?:,|\))', content, re.DOTALL)
                    # For this.$t('key') or this.$t("key") or this.$t(`key`)
                    matches3 = re.findall(r'this\.\$t\s*\(\s*["`\']([a-zA-Z][a-zA-Z0-9_.]*)["`\']\s*(?:,|\))', content, re.DOTALL)
                    # For i18n.global.t('key') or i18n.global.t("key") or i18n.global.t(`key`)
                    matches6 = re.findall(r'i18n\.global\.t\s*\(\s*["`\']([a-zA-Z][a-zA-Z0-9_.]*)["`\']\s*(?:,|\))', content, re.DOTALL)
                    # For {{ $t('key') }} in template
                    matches4 = re.findall(r'{{[^}]*\$t\s*\(\s*["`\']([a-zA-Z][a-zA-Z0-9_.]*)["`\']\s*\)[^}]*}}', content, re.DOTALL)
                    # For {{ t('key') }} in template with word boundary
                    matches5 = re.findall(r'{{[^}]*\bt\s*\(\s*["`\']([a-zA-Z][a-zA-Z0-9_.]*)["`\']\s*\)[^}]*}}', content, re.DOTALL)
                    all_matches = matches1 + matches2 + matches3 + matches4 + matches5 + matches6
                    for match in all_matches:
                        if match and len(match) > 1 and match[0].isalpha():  # Filter: starts with letter, length >1
                            used_keys.add(match)
    return used_keys
...
def main():
    ...
    for key in defined_keys:
        if key.startswith('chat.date.') or \
        key.startswith('menu.') or \
        key.startswith('settings.skill.type.') or \
    ...
    with open(output_file, "w", encoding='utf-8') as f:
        f.write("# 前端 i18n 检查结果 (合并 zh-Hans.json 和 copy_from_rust_src_i18n.json)\n\n")
