...
test('terminal preferences bound output, preserve terminal input, and use detected shell choices', async () => {
  ...
  assert.match(panel, /\.workflow-terminal__content \{[^}]*padding: var\(--cs-space-sm\) 0 0 var\(--cs-space-sm\)[^}]*box-sizing: border-box/s)
  assert.match(panel, /\.workflow-terminal__content \{[^}]*background: var\(--workflow-terminal-background\)[^}]*\}/s)
  assert.match(panel, /\.workflow-terminal__content :deep\(canvas\) \{[^}]*width: 100%[^}]*height: 100%/s)
