...
light_dest_dir = './public/highlight.js/light'
dark_dest_dir = './public/highlight.js/dark'
...
ignore_name = ['nncs-light','windows-nt-light','windows-high-contrast-light','windows-95-light','windows-10-light']
...
def run_copy_style():
    ...
    for filename in os.listdir(source_dir):
        ...
        if new_filename in ignore_name:
            continue
        if (is_dark_style(filename) and new_filename not in light_styles) or new_filename in dark_styles:
            # Copy dark styles to the dark directory
            shutil.copy(os.path.join(source_dir, filename), os.path.join(dark_dest_dir, new_filename + '.css'))
            dark_files.append(new_filename)
        elif filename.endswith('.min.css'):
            # Copy light styles to the light directory
            shutil.copy(os.path.join(source_dir, filename), os.path.join(light_dest_dir, new_filename + '.css'))
    ...
    if os.path.exists(base16_dir):
        for filename in os.listdir(base16_dir):
            ...
            if (is_dark_style(filename) and new_filename not in light_styles) or new_filename in dark_styles:
                # Copy dark styles to the dark directory
                shutil.copy(os.path.join(base16_dir, filename), os.path.join(dark_dest_dir, new_filename + '.css'))
                dark_files.append(new_filename)
            elif filename.endswith('.min.css'):
                # Copy light styles to the light directory
                shutil.copy(os.path.join(base16_dir, filename), os.path.join(light_dest_dir, new_filename + '.css'))
                light_files.append(new_filename)

    # generate the json file
    os.makedirs('./src/config/highlight.js', exist_ok=True)
    with open('./src/config/highlight.js/themes.json', 'w') as f:
        json.dump({'light': sorted(list(set(light_files))), 'dark': sorted(list(set(dark_files)))}, f)
