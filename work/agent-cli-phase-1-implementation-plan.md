...
- V-7：**未执行**。本实施环境为无显示服务器的 headless 沙箱（无 DISPLAY/Wayland，主进程未运行），无法启动 `pnpm tauri dev` 进行 UI/CLI 同 session 交叉 smoke；付费模型 smoke 亦未执行。这是第一期唯一未验证项。CLI 侧的"主进程未运行"失败路径已按 AC-2 实测（见 V-5）。
