...
# Detect target architecture (allow overrides via environment variables)
$targetArch = $env:PROCESSOR_ARCHITECTURE
$overrideTriplet = if ($env:CHAT_SPEED_VCPKG_TRIPLET) { $env:CHAT_SPEED_VCPKG_TRIPLET } else { $env:VCPKG_DEFAULT_TRIPLET }
$overrideBuildArch = $env:CHAT_SPEED_BUILD_ARCH

if ($overrideTriplet -or $overrideBuildArch) {
    if ($overrideTriplet) {
        $vcpkgTriplet = $overrideTriplet
    }
    if ($overrideBuildArch) {
        $buildArch = $overrideBuildArch
    }
} else {
...
Write-Host "Detected architecture: $targetArch, using triplet: $vcpkgTriplet, build arch: $buildArch"
