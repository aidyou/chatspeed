...
- **工作流模块正式加入 ChatSpeed**：`2.0.0` 引入了全新的 Workflow 模块。这不是对原有聊天链路的小修小补，而是一次新的任务执行能力扩展：支持计划与执行分层、工具驱动式任务推进、审批流、子智能体协作、状态恢复、上下文压缩、记忆提取，以及与 Skills / MCP / AGENTS.md 的深度集成。
...
- **全新 Workflow 工作流引擎**：
  - 新增独立的任务型工作流界面，支持与普通聊天分离的长程任务执行。
...
- **工作流上下文系统**：
  - 新增结构化事件流、快照、重放恢复与热恢复能力，提升异常中断后的可恢复性。
  - 引入 **智能分级上下文压缩**：会按任务边界、已完成任务、当前任务状态进行结构化压缩，尽量保留对当前目标真正有用的信息，避免简单截断带来的上下文污染。
  - 在编程任务场景下，这套压缩链路可显著提升长任务续航表现；以 `deepseek-v4` 为例，实际测试中上下文输入缓存命中率可达到约 **96%**。
  - 新增项目记忆、全局记忆、任务状态摘要等机制，改善长任务持续运行体验。
- **内置智能体体系**：
  - 新增内置主智能体与子智能体定义机制，支持随版本同步发布、升级与扩展。
  - 内置智能体支持保留官方提示词与角色定义，同时允许用户配置模型、工具、安全策略、技能等运行参数。
- **工作流安全与可控执行能力**：
  - 新增工具白名单、自动审批、Shell Policy、授权目录与路径校验能力。
  - 新增审批流、继续执行、用户追问、运行时配置更新等完整信号控制链路。
  - 新增 **多智能体审批智能提醒**：在多个工作流、多个智能体并发运行时，系统可对待处理审批进行更清晰的提醒与聚合展示，降低遗漏风险。
- **工作流交互与配置体验**：
  - 新增工作流专属的模型、技能、状态、审批与工具展示界面。
  - 新增工作流内 Skills 启用与选择、内置智能体、模型状态面板和 Slash Commands 等能力。
  - 支持 MCP 工具接入、Skills 组合、Shell 配置与智能体配置协同，让工作流既能用系统内能力，也能挂接外部工具生态。
  - 新增面向工作流智能体的图像识别支持，让截图等视觉输入可以更自然地进入执行型任务链路。
