...
- **极致启动鲁棒性**：重构了初始化序列为完全异步且非阻塞模式。环境路径检测、工具注册及本地 HTTP 服务器等非核心任务现在会延迟到 UI 启动后执行。这确保了即使在完全断网或受限防火墙环境下，应用也能秒开且永不闪退。
...
- **敏感信息模块健康感知**：敏感信息管理器现在能优雅地处理初始化失败（如正则编译问题）。模块不再会导致应用闪退，而是向 UI 报告健康状态，允许前端禁用相关功能并向用户显示清晰的诊断信息。
...
- **跨协议负数索引崩溃修复**：修复了一个关键的解析 Bug，即当 Qwen 等模型在工具调用或嵌入向量中返回 `index: -1` 时会导致程序 Panic。通过将全协议相关的 `index` 字段类型从 `u32` 迁移至 `i32`，实现了对各类非标 OpenAI 实现的最大化兼容。
- **代理日志标签校准**：修正了代理模块的调试日志，明确区分了“后端原始响应”与“转换后客户端响应”，解决了跨协议代理时日志内容容易引起歧解的问题。
- **Windows 11 启动崩溃修复**：通过回退某些与特定线程模型冲突的 SQLite 打开标志（`SQLITE_OPEN_FULL_MUTEX`），解决了在 Windows 11 生产环境下的 Panic 问题。
- **SQL 执行错误修复**：修复了 `rusqlite` 在执行带结果行的 `PRAGMA` 指令时报“Execute returned results”的错误。
- **所有权编译错误**：修复了异步块中 `Arc` 移动语义导致的代码编译失败。
- **脚本编码清理**：清理了 Windows PowerShell 脚本中潜在的编码问题及不可见字符，解决了编译时报“聽”指令未找到的问题。
...
- **MCP SSE 协议弃用**：`/mcp/sse` 入口已正式弃用，并计划于 **v1.3.0** 版本彻底删除。由于 SSE 协议存在难以修复的稳定性问题（如 410 错误）且上游依赖库已停止支持，强烈建议用户尽快迁移至更稳定的 `/mcp/http` (Streamable HTTP) 协议。
